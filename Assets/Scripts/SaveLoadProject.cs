...
public class SaveLoadProject : MonoBehaviour {
    ...
    private static extern IntPtr FreeImage_GetVersion();
    ...
    void Start () {
        ...
        try {
        IntPtr versionPtr = FreeImage_GetVersion();
        string version = Marshal.PtrToStringAnsi(versionPtr);
        UnityEngine.Debug.Log("Version: " + version);
    } catch (Exception e) {
        UnityEngine.Debug.LogError("Version unknow: " + e.Message);
    }
    ...
    public IEnumerator SaveTexture( string extension, Texture2D textureToSave, string pathToFile ) {
        ...
        if (textureToSave != null) {
            ...
            if( useFIF ){
                ...
                string tempFilePath = Application.temporaryCachePath + pathChar + "temp.png";
                File.WriteAllBytes (tempFilePath, bytes);

                FreeImage_Initialise(0);
                IntPtr bitmap = FreeImage_Load (FREE_IMAGE_FORMAT.FIF_PNG, tempFilePath, 16);
                
                if (bitmap != IntPtr.Zero) {
                    int saveSuccess = FreeImage_Save (imageFormat, bitmap, pathToFile + "." + extension, (int)flags );
    ...
    public IEnumerator LoadTexture( MapType textureToLoad, string pathToFile ) {
        ...
        string extension = pathToFile.Substring (fileIndex+1, pathToFile.Length-fileIndex-1).ToLower();

        bool loadSuccess = false;
        string newPathToFile = Application.temporaryCachePath + pathChar + "temp_load.png";
        ...
        if (extension == "png" || extension == "jpg" || extension == "jpeg") {
            ...
            try {
                ...
                IntPtr bitmap = FreeImage_Load (imageFormat, pathToFile, 16);

                if (bitmap != IntPtr.Zero) {
                    int saved = FreeImage_Save (FREE_IMAGE_FORMAT.FIF_PNG, bitmap, newPathToFile, 0);
                    loadSuccess = (saved == 1);
       ...
       if (loadSuccess) {
            ...
            string fileUrl = new System.Uri(newPathToFile).AbsoluteUri;
            ...
            using (UnityEngine.Networking.UnityWebRequest uwr = UnityEngine.Networking.UnityWebRequestTexture.GetTexture(fileUrl)) {
                ...
                if (!string.IsNullOrEmpty(uwr.error)) {
                    UnityEngine.Debug.Log(uwr.error + " | URL: " + fileUrl);
                } else {
                    newTexture = UnityEngine.Networking.DownloadHandlerTexture.GetContent(uwr);
