...
private static extern IntPtr FreeImage_ConvertTo24Bits(IntPtr dib);

[DllImport ("FreeImage", CallingConvention = CallingConvention.Cdecl)]
private static extern IntPtr FreeImage_Clone(IntPtr dib);

[DllImport ("FreeImage", CallingConvention = CallingConvention.Cdecl)]
private static extern void FreeImage_Initialise(int load_local_plugins_only);

[DllImport ("FreeImage", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
private static extern IntPtr FreeImage_Load(FREE_IMAGE_FORMAT fif, string filename, int flags);

[DllImport ("FreeImage", CallingConvention = CallingConvention.Cdecl)]
private static extern void FreeImage_Unload(IntPtr dib);

[DllImport ("FreeImage", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
private static extern int FreeImage_Save(FREE_IMAGE_FORMAT fif, IntPtr dib, string filename, int flags);
...
[DllImport ("FreeImage", CallingConvention = CallingConvention.Cdecl)]
private static extern int FreeImage_GetPixelColor(IntPtr dib, int x, int y, ref RGBQUAD value);

[DllImport ("FreeImage", CallingConvention = CallingConvention.Cdecl)]
private static extern IntPtr FreeImage_MakeThumbnail(IntPtr dib, int max_pixel_size, int convert);
    ...
    string MakeNewThumb(string filename, string tempName, string fileLower, int size) {
        if (fileLower.EndsWith(".mtz")) {
            ...
            if(File.Exists(imageFileName + "_diffuseOriginal.bmp")) imageFileName += "_diffuseOriginal.bmp";
            else if(File.Exists(imageFileName + "_diffuse.bmp")) imageFileName += "_diffuse.bmp";
            else if(File.Exists(imageFileName + "_diffuseOriginal.jpg")) imageFileName += "_diffuseOriginal.jpg";
            else if(File.Exists(imageFileName + "_diffuse.jpg")) imageFileName += "_diffuse.jpg";
            else if(File.Exists(imageFileName + "_diffuseOriginal.png")) imageFileName += "_diffuseOriginal.png";
            else if(File.Exists(imageFileName + "_diffuse.png")) imageFileName += "_diffuse.png";
            else if(File.Exists(imageFileName + "_diffuseOriginal.tga")) imageFileName += "_diffuseOriginal.tga";
            else if(File.Exists(imageFileName + "_diffuse.tga")) imageFileName += "_diffuse.tga";
            else if(File.Exists(imageFileName + "_diffuseOriginal.tif")) imageFileName += "_diffuseOriginal.tif";
            else if(File.Exists(imageFileName + "_diffuse.tif")) imageFileName += "_diffuse.tif";
            else if(File.Exists(imageFileName + "_diffuseOriginal.tiff")) imageFileName += "_diffuseOriginal.tiff";
            else if(File.Exists(imageFileName + "_diffuse.tiff")) imageFileName += "_diffuse.tiff";
            else return "null";
        ...
        if (fileLower.EndsWith(".tif") || fileLower.EndsWith(".tiff")) {
            return "null";
        }

        string newFilename = Application.temporaryCachePath + "/" + tempName + ".png";

        try {
            FreeImage_Initialise(0);
            
            FREE_IMAGE_FORMAT imageFormat = FREE_IMAGE_FORMAT.FIF_UNKNOWN;
            if(fileLower.EndsWith(".bmp")) imageFormat = FREE_IMAGE_FORMAT.FIF_BMP;
            else if(fileLower.EndsWith(".tga")) imageFormat = FREE_IMAGE_FORMAT.FIF_TARGA;
            
            IntPtr fiBitmap = FreeImage_Load(imageFormat, filename, 16);

            if (fiBitmap != IntPtr.Zero) {
                IntPtr fiThumbnail = FreeImage_MakeThumbnail(fiBitmap, size, 1);
                
                if (fiThumbnail != IntPtr.Zero) {
                    if(File.Exists(newFilename)) {
                        File.SetAttributes(newFilename, FileAttributes.Normal);
                    }
                    FreeImage_Save(FREE_IMAGE_FORMAT.FIF_PNG, fiThumbnail, newFilename, 0);
    ...
    IEnumerator loadIcons()
    {
        for (int i = 0; i < this.filesWithImages.Length; i++) {
            ...
            if( filename != "null" ){
                ...
                string fileUrl = new System.Uri(filename).AbsoluteUri;

                using (UnityEngine.Networking.UnityWebRequest uwr = UnityEngine.Networking.UnityWebRequestTexture.GetTexture(fileUrl)) {
                    yield return uwr.SendWebRequest();

                    if (string.IsNullOrEmpty(uwr.error)) {
                        Texture2D downloadedTex = UnityEngine.Networking.DownloadHandlerTexture.GetContent(uwr);
                        
                        if (downloadedTex != null) {
                            GUIContent item = this.filesWithImages[i];
                            Texture2D tex = new Texture2D(downloadedTex.width, downloadedTex.height, TextureFormat.ARGB32, false, true);
                            
                            tex.SetPixels(downloadedTex.GetPixels());
                            tex.Apply();
            ...
            if (GUILayout.Button("Confirm", this.styleDirsTopBut, GUILayout.Width(80))) {
                ...
                if (this.currentDirectory == "Favorites") {
                    ...
                    try {
                        ...
                        UnityEngine.Debug.LogError("Errore eliminazione: " + e.Message);
                    }
                }
...
IEnumerator loadPreview(string filename)
    {
        ...
        if ( isMaterial || isImage ) {
            ...
            if( newFilename != "null" ){
                string fileUrl = new System.Uri(newFilename).AbsoluteUri;
                using (UnityEngine.Networking.UnityWebRequest uwr = UnityEngine.Networking.UnityWebRequestTexture.GetTexture(fileUrl)) {
                    yield return uwr.SendWebRequest();
                    if (string.IsNullOrEmpty(uwr.error)) {
                        this.previewTexture = UnityEngine.Networking.DownloadHandlerTexture.GetContent(uwr);
                    }
                }
            }
            yield break;
        ...
        if ( isSound ) {
            string fileUrl = new System.Uri(filename).AbsoluteUri;
            using (UnityEngine.Networking.UnityWebRequest uwr = UnityEngine.Networking.UnityWebRequestMultimedia.GetAudioClip(fileUrl, AudioType.UNKNOWN)) {
                yield return uwr.SendWebRequest();
                if (string.IsNullOrEmpty(uwr.error)) {
                    this.UpdateSoundPreviewRect();
                    this.soundPreviewer.enabled = true;
                    this.soundPreviewer.configs = this.soundPreviewOptions;
                    this.soundPreviewer.PreviewClip(UnityEngine.Networking.DownloadHandlerAudioClip.GetContent(uwr));
                }
            }
            yield break;
        }
    ...
    IEnumerator __GetTexture(string path, TextureLoadedCallback callback)
    {
        ...
        string fileUrl = new System.Uri(path).AbsoluteUri;
        ...
        using (UnityEngine.Networking.UnityWebRequest uwr = UnityEngine.Networking.UnityWebRequestTexture.GetTexture(fileUrl)) {
            ...
            if (string.IsNullOrEmpty(uwr.error)) {
                Texture2D downloadedTex = UnityEngine.Networking.DownloadHandlerTexture.GetContent(uwr);
                if (downloadedTex != null) {
                    Texture2D texture = new Texture2D(downloadedTex.width, downloadedTex.height, TextureFormat.ARGB32, false, true);
                    texture.wrapMode = TextureWrapMode.Clamp;
                    texture.SetPixels(downloadedTex.GetPixels());
                    texture.Apply();
